...
export const getCalendarByUser = async ({ uid }: { uid: string }): Promise<CalendarioType> => {
  ...
  return getDoc(docRef)
    .then(docSnap => {
      ...
      return (agenda === null || agenda === undefined || agenda.length === 0) ? defaultCalendar : agenda;
    })
    .catch(error => {
...
export const updateCalendarByUser = async ({ uid, agenda, }: { uid: string; agenda: CalendarioType; }) => {
  try {
